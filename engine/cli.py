...
def _select_before_relaunch(name: str, health: dict | None) -> str | None:
    ...
    * the PUT was refused — no such session, or the proxy stopped answering. `_control` has
    ...
    if problems is None:
        click.echo(f"{RED}✗ the running proxy is older than this CLI and cannot say whether "
                   f"'{name}' loaded whole — the app was NOT relaunched against a session "
                   f"nothing could check.{R}\n"
    ...
    if problems:
        ...
        click.echo(f"{RED}✗ session '{name}' did not load whole: {detail}{R}\n"
...
def _up_locked(bundle_id: str | None, no_relaunch: bool = False, use_name: str | None = None,
    ...
    try:
        simulator: Simulator | None = resolve_simulator(simulator_selector)
    except SimulatorError as error:
        simulator = None
        click.echo(f"{RED}✗ simulator: {error}{R}\n"
                   f"   the CA was NOT trusted and nothing was relaunched.")
        failures.append(f"no simulator to work on: {str(error).splitlines()[0]}")

    if simulator is not None:
        ca_ok, message = trust_ca_in_sim(simulator)
    ...
    if as_json:
        ...
        if simulator:
            click.echo(f"  simulator: {simulator.get('name')} ({simulator.get('udid')}) "
                       f"{DIM}· CA + relaunch only; the PAC is not scoped to it{R}")
...
#
# Reading a session file, never the proxy. Nothing here starts a process, calls the control API,
# touches network settings or constructs a `Store` — a store creates directories, synthesises a
# `default` session and reads the active-session pointer, all of which are changes to a profile
# somebody else may be using. `store.load_session_file` is the same function startup loads with,
# so what these commands report is what the proxy would do, not a second opinion about it.
...
def validate(name: str | None, as_json: bool) -> None:
    ...
    for report in reports:
        ...
        if report["ok"]:
            summary = f"{GREEN}✓{R} {label}{DIM}{count} rule(s){R}"
        elif report["loaded"]:
            summary = (f"{RED}✗{R} {label}{DIM}{count} rule(s) kept, "
                       f"{len(report['problems'])} dropped{R}")
        else:
            summary = f"{RED}✗{R} {label}{DIM}not loaded at all{R}"
        click.echo(summary)
        for problem in report["problems"]:
            click.echo(f"    {DIM}{problem}{R}")

    if not ok:
        if reports:
            click.echo(f"{RED}✗ {sum(1 for r in reports if not r['ok'])} of {len(reports)} "
                       f"session(s) cannot be accepted whole{R}")
        raise SystemExit(1)
    click.echo(f"{GREEN}✓ {len(reports)} session(s) load whole{R}")
...
def explain_match(method: str, path: str, body: str, session_name: str | None, as_json: bool) -> None:
    ...
    if problems:
        ...
        click.echo(f"  {RED}dropped at load, so not in the ranking above:{R}")
        for problem in problems:
            click.echo(f"    {DIM}{problem}{R}")
...
def relaunch_cmd(bundle_id: str | None, simulator_selector: str | None) -> None:
    ...
    if not target:
        click.echo(f"{RED}✗ nothing to relaunch: pass a bundle id, or set simBundleId in "
                   f"{config.PROFILE_FILE}{R}")
        raise SystemExit(1)
    try:
        simulator = _bound_simulator(simulator_selector)
    except SimulatorError as error:
        click.echo(f"{RED}✗ {error}{R}")
        raise SystemExit(1) from None
    launched, detail = _relaunch(target, simulator)
    click.echo(f"{'✓' if launched else '✗'} relaunch {target}: {detail}")
    if not launched:
