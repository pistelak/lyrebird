...
final class BadgeView: NSView {
    ...
    init(_ text: String, tint: NSColor? = nil, circle: Bool = false) {
        self.tint = tint
        self.circle = circle
        label = NativeStyle.label(
            text, font: circle ? .systemFont(ofSize: 11, weight: .regular) : BrowserTypography.badge)
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        if circle {
            ...
            NSBezierPath(roundedRect: rect, xRadius: BrowserMetrics.badgeRadius, yRadius: BrowserMetrics.badgeRadius)
                .fill()
        }
    }
